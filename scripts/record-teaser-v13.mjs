/**
 * Teaser_v12.mov → Teaser_v13.mov
 * Sustituye el cierre estático por título + créditos animados (MORVO-Final).
 *
 * Uso: node scripts/record-teaser-v13.mjs
...
const SRC_MOV = path.join(os.homedir(), "Downloads", "Teaser_v12.mov");
const OUT_FULL = path.join(os.homedir(), "Downloads", "Teaser_v13.mov");
const OUT_CREDITS = path.join(os.homedir(), "Downloads", "Teaser_v13_credits-end.mov");

/** Final de la máscara — antes del pasillo con créditos quemados (~59.7 s) */
const CUT_AT_SEC = 59.55;
/** Hold de créditos animados (estilo MORVO-Final) */
...
async function main() {
  ...
  const work = fs.mkdtempSync(path.join(os.tmpdir(), "v13-credits-"));
  ...
  // Recorte preciso (re-encode) — evita arrastrar frames del pasillo por keyframes
  runFfmpeg(
    ffmpeg,
    [
      "-y",
    ...
    "Cabeza Teaser_v12 (recorte preciso)",
  );
